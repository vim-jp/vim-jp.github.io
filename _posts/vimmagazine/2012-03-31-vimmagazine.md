---
layout: post
category: vimmagazine
title: Vim Magazine 3 月号
redirect_from:
  - /blog/2012/03/31/vimmagazine.html
  - /2012/03/31/vimmagazine.html
---

## 話題

### [vim\_dev: Google summer of code mentor](http://groups.google.com/group/vim_dev/browse_thread/thread/5e806ba0ecfb0283)

Google Summer of Code への参加に向けて準備が進められています。小規模でや
りたいとのことで生徒の募集は 3 人程度になりそうです。


## リリース情報

- [7.3.465 : cannot get file name with newline from glob()](https://github.com/vim/vim/commit/146e9c38866248ff92f6109c536bbe79744256f5)
- [7.3.466 : ml\_get error when replacing several selected lines](https://github.com/vim/vim/commit/9e98edfe8f8503a3ea84da714bbb5ef37364199e)
- [7.3.467 : cursor position wrong at command line when using input method](https://github.com/vim/vim/commit/917ba89f7f787873cbfc9ce2e8a20b948843f0ae)
- [7.3.468 : for some compilers the error file is not easily readable](https://github.com/vim/vim/commit/8ec1f8523b309aa9eb52e2854fc9a4b9559ebe80)
- [7.3.469 : compiler warning for unused argument](https://github.com/vim/vim/commit/5fbe6993903ed3e8d7c288b337c1b0eaf2e14bd8)
- [7.3.470 : test 62 fails when compiled without GUI and X11](https://github.com/vim/vim/commit/7c578d3c3f501c0dd86c48aad6816a6ce6e3fc77)
- [7.3.471 : can't abort listing placed signs](https://github.com/vim/vim/commit/1c0b03e857c3f0eb9201f1046150a63866ebe49e)
- [7.3.472 : crash when using ":redraw" in a BufEnter autocmd](https://github.com/vim/vim/commit/5ad15df9ce49f0b7adeac2c8387319d6f309bd5f)
- [7.3.473 : 'cursorbind' does not work correctly with 'virtualedit' "all"](https://github.com/vim/vim/commit/1ea69b7077fe26466210bd00bb20c35be84cca40)
- [7.3.474 : gcc 4 build with Perl fails](https://github.com/vim/vim/commit/dfba6b64495d389c9aec04f050ca7cbab01c0438)
- [7.3.475 : in terminal with few colors omnicomplete menu is hard to see](https://github.com/vim/vim/commit/5d3a8038b6a59e6f1b219f27ec77d6067e643ac1)
- [7.3.476 : backspacing in a Visual block selection causes problems.](https://github.com/vim/vim/commit/5e4b9e9ff8ebd337875f918078170ebbad03be9a)
- [7.3.477 : scrolling back at the more prompt shows the wrong text](https://github.com/vim/vim/commit/12b0290d42efe9b6cf93f2903f12d76a100c60cb)
- [7.3.478 : memory leak when the ':rv!' command reads a dict or list](https://github.com/vim/vim/commit/507cc8acf1175d7d15abc004ace214dfeffc031c)
- [7.3.479 : 'cursorline' line number highlighting can't be set separately](https://github.com/vim/vim/commit/06ca513e2c25e2e2f63580f7d3d741236e17a19e)
- [7.3.480 : when using ":qa" priority of changed buffers could be better](https://github.com/vim/vim/commit/970a1b82ecb74f4b89f5da71333d04ef27bbf075)


## 新着スクリプト

- [fwk notes - This plugin was created to manages your task/notes by days/weeks/years.](http://www.vim.org/scripts/script.php?script_id=3956)
- [calendar.vim - calendar 日历 农历](http://www.vim.org/scripts/script.php?script_id=3957)
- [mystatusinfo.vim - Display Current Tab info on status line](http://www.vim.org/scripts/script.php?script_id=3958)
- [workflowish - ToDo manager plugin inspired by workflowy.com](http://www.vim.org/scripts/script.php?script_id=3959)
- [tabman.vim - Tab management for Vim](http://www.vim.org/scripts/script.php?script_id=3960)
- [flashdevelop.vim - FlashDevelop features for VIM](http://www.vim.org/scripts/script.php?script_id=3961)
- [hammer.vim - Transform your markup language of choice to HTML, and open the transformed HTML.](http://www.vim.org/scripts/script.php?script_id=3962)
- [Colorizer - highlight color codes and names](http://www.vim.org/scripts/script.php?script_id=3963)
- [less - less syntax for vim](http://www.vim.org/scripts/script.php?script_id=3964)
- [DynamicSigns - Use Signs for different things](http://www.vim.org/scripts/script.php?script_id=3965)
- [ShowTrailingWhitespace - Detect unwanted whitespace at the end of lines.](http://www.vim.org/scripts/script.php?script_id=3966)
- [DeleteTrailingWhitespace - Delete unwanted whitespace at the end of lines.](http://www.vim.org/scripts/script.php?script_id=3967)
- [JumpToTrailingWhitespace - Motions to locate unwanted whitespace at the end of lines.](http://www.vim.org/scripts/script.php?script_id=3968)
- [source.vim - source vim plugins and remote scripts](http://www.vim.org/scripts/script.php?script_id=3969)
- [mediawiki.vim - This holds a syntax highlighter for MediaWiki-based projects, mostly Wikipedia.](http://www.vim.org/scripts/script.php?script_id=3970)
- [regexroute.vim - Syntax Highlighting for Yate](http://www.vim.org/scripts/script.php?script_id=3971)
- [css3 syntax plus - A lightweight css3 syntax file for vim](http://www.vim.org/scripts/script.php?script_id=3972)
- [diff-toggle - Toggle diff lines](http://www.vim.org/scripts/script.php?script_id=3973)
- [showmarks2 - bookmark works like MS utilities](http://www.vim.org/scripts/script.php?script_id=3974)
- [Finder for vim - Finder for VIm](http://www.vim.org/scripts/script.php?script_id=3975)
- [vim-human-dates - Wouldn't it be nice if you could view epoch dates in a format that made sense?](http://www.vim.org/scripts/script.php?script_id=3976)
- [vim-addon-commenting - minimal commenting plugin](http://www.vim.org/scripts/script.php?script_id=3977)
- [cudajinja.vim - Jinja templating syntax for CUDA](http://www.vim.org/scripts/script.php?script_id=3978)
- [vim-pomodoro - Vim plugin for the Pomdoro time management technique](http://www.vim.org/scripts/script.php?script_id=3979)


## vim-jp issues

Open : 111 (+3) | Closed : 70 (+4)

- [Issue #175 : ＠ITの「いまさら聞けないVim」の記事の質を向上させたい](https://github.com/vim-jp/issues/issues/175)
- [Issue #176 : InsertCharPre で "\\\<BS>" や "\\\<Left>" も動くと嬉しい](https://github.com/vim-jp/issues/issues/176)
- [Issue #177 : DLLを複数環境で共有していると問題が発生するかもしれないケースについて考える](https://github.com/vim-jp/issues/issues/177)
- [Issue #178 : pathogen, vundle, neobundleを利用したとき、インストールしたプラグインが読み込まれない](https://github.com/vim-jp/issues/issues/178)
- [Issue #179 : There is not the way to set executed external command encoding.](https://github.com/vim-jp/issues/issues/179)
- [Issue #180 : mingwでif\_perlのビルドに失敗する](https://github.com/vim-jp/issues/issues/180)
- [Issue #181 : タスクバーが上部にある場合にウィンドウ位置が不正になる](https://github.com/vim-jp/issues/issues/181)


## 付録

[Vim ロゴ入り栞](https://github.com/downloads/vim-jp/vim-jp.github.com/bookmark-vim1.pdf)

A4 用紙に印刷して切り取ってお使いください。普通紙なら 2 枚分切り取って半分に折ってのりでくっつけると両面になって厚みもでて良いかもしれません。

